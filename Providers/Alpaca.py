...
class Alpaca:
    ...
    def getStockPrices(self,symbols=None,timeframe='1D',start=None,end=None,limit=None,output_as=None):
        ...
        try:
            ...
            if output_as==None:
                stock_prices=[self.connection.get_barset(symbols,timeframe=timeframe,limit=limit,start=start,end=end) for symbols in stock_list]

            else:

                stock_prices=[self.connection.get_barset(symbols,timeframe=timeframe,limit=limit,start=start,end=end).df for symbols in stock_list].df
